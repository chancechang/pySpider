...
def main6(ty,page):
    f1=open('P_detail_final'+str(ty)+'.csv','w+',newline='',encoding='gb18030')
    csv_write1=csv.writer(f1)
    found=True
    for line in open('P_detail'+str(ty)+'.txt','r',encoding='utf-8'):
...
def main4(ty,filename,workbook,page):
    ...
    for line in open('zupushuid'+str(ty)+'.txt','r',encoding='utf-8'):
        line=eval(line)
        if line[0]=='0':
            continue
    ...
    # './zupu1/P_detail'+str(ty)+'.csv'
    main6(ty,page)
    try:
        df1 = pd.DataFrame(pd.read_csv('P_detail_final'+str(ty)+'.csv',header=None,encoding='gb18030',))
    except:
        main6(ty,page)
        try:
            df1 = pd.DataFrame(pd.read_csv('P_detail_final'+str(ty)+'.csv',header=None,encoding='gb18030',))
    ...
    # print(df1.head())
    r=2
    for id_format  in open('zupushuid'+str(ty)+'.txt','r'):
        col=-1
...
def main8():
    ...
    for line in open(zupufile+'.txt','r',encoding='utf-8'):
        ...
        main4(ty,zupuname,workbook,page)
        j=j+1 
        if j==num:
            j=0
            workbook.close()
            page=page+1
            print('page'+str(page))
            workbook = xlsxwriter.Workbook('excel_page'+str(page)+'.xlsx')
        if page==page_max:
            break
            # break  
    workbook.close()
