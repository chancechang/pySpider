...
def main6(ty):
 ...
 #族谱id,wookbook,写入的行数
def main4(ty,workbook,worksheet_1,r):
    ...
    # 序号和世都写上
    worksheet_1.write(r,0,'族谱树链接',)
    tree_link='http://zupu.syshenshi.com/ZPTree.aspx?infotype'+str(ty)
    worksheet_1.write(r,1,tree_link,)
    worksheet_1.write(r+1,0,'序号',)#'A2'
    worksheet_1.write(r+1,1,'世',)#'B2'
    # r=i
    for j in range(0,colMax):
        # print(j)
        worksheet_1.merge_range(r,j*33+2,r,j*33+34,rowname[j],yellow)
        for i in range(33):
            worksheet_1.write(r+1,j*33+2+i,personalrowname[i].replace('：',''))
...
def main8():
    ...
    for line in open(zupufile+'.txt','r',encoding='utf-8'):
        ...
        # ty='85'
        print(ty)    
        r=main4(ty,workbook,worksheet_1,r) #族谱id,wookbook,写入的行数
        j=j+1 
        if j==num:
            j=0
            workbook.close()
